...
class Session
{
        ...
        post ( method: "friends.get", queries: [] ) {[completion] ( json ) in
            if ( json.count > 0 )
            {
                ...
                let queries = [
                    ...
                    URLQueryItem ( name: "fields", value: "first_name, last_name, photo_id" )
                ]
                self.post ( method: "users.get", queries: queries ) { [completion] ( json ) in
                    ...
                    for item in json.arrayValue
                    {
                        ...
                        let friend = Friend ( _firstName: firstName, _lastName: lastName )
        ...
        post ( method: "groups.get", queries: queries ) {[completion] ( json ) in
            ...
            if ( json.count > 0 ) {
                ...
                for item in groupArray {
                    groups.append ( Group ( _groupName: item ["name"].stringValue ))
                }
            }
            completion ( groups )
        }
    ...
    public func receivePhotos ( _ photoId: String, completion: @escaping ( JSON ) -> Void )
    {
        post ( method: "photos.getById", queries: [URLQueryItem ( name: "photos", value: photoId )], completion: completion )
    }
